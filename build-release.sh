...
for os in ${OSES[@]}; do
	for arch in ${ARCHS[@]}; do
		...
		env CGO_ENABLED=1 GOOS=$os GOARCH=$arch go build -ldflags "$LDFLAGS" -gcflags "$GCFLAGS" -o pt-rss_${os}_${arch}${suffix} github.com/tominescu/pt-rss/pt-rss
...
ARMS=(5 6 7)
for v in ${ARMS[@]}; do
	env CGO_ENABLED=1 GOOS=linux GOARCH=arm GOARM=$v go build -ldflags "$LDFLAGS" -gcflags "$GCFLAGS" -o pt-rss_linux_arm$v  github.com/tominescu/pt-rss/pt-rss
done
